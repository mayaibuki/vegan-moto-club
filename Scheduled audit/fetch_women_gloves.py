"""
fetch_women_gloves.py
Fetch REV'IT! women's gloves, dedup by base name, then visit each product
...
r = requests.get(
    'https://revitsport.com/en-us/collections/women-motorcycle-gloves/products.json',
...
out = os.path.join(os.path.dirname(__file__), 'women_gloves.json')
with open(out, 'w') as f:
    json.dump(vegan, f, indent=2, ensure_ascii=False)
print(f'\nVegan women gloves: {len(vegan)} -> {out}')
