"""
fetch_layers.py
Fetch REV'IT! men's + women's base/mid layers, dedup by base name, visit each
product page and check the Sustainability tab for the explicit <h2>Vegan</h2> marker.
"""
...
COLLECTIONS = {
    'men':   'https://revitsport.com/en-us/collections/men-base-mid-layers/products.json',
    'women': 'https://revitsport.com/en-us/collections/women-base-mid-layers/products.json',
}
...
def process(gender, url):
    ...
    print(f'[{gender}] unique layers: {len(unique)}')
    out = []
...
out = os.path.join(os.path.dirname(__file__), 'layers.json')
with open(out, 'w') as f:
    json.dump(allv, f, indent=2, ensure_ascii=False)
print(f'\nVegan layers total: {len(allv)} -> {out}')
