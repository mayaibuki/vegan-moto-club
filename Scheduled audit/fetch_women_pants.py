"""
fetch_women_pants.py
Fetch REV'IT! women's pants, dedup by base name, then visit each product page
...
r = requests.get(
    'https://revitsport.com/en-us/collections/women-motorcycle-pants/products.json',
...
print(f'Unique pants: {len(unique)}')
...
out = os.path.join(os.path.dirname(__file__), 'women_pants.json')
with open(out, 'w') as f:
    json.dump(vegan, f, indent=2, ensure_ascii=False)
print(f'\nVegan women pants: {len(vegan)} -> {out}')
