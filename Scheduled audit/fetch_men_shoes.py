"""
fetch_men_shoes.py
Fetch REV'IT! men's shoes, dedup by base name, then visit each product page
...
r = requests.get(
    'https://revitsport.com/en-us/collections/men-motorcycle-shoes/products.json',
...
out = os.path.join(os.path.dirname(__file__), 'men_shoes.json')
with open(out, 'w') as f:
    json.dump(vegan, f, indent=2, ensure_ascii=False)
print(f'\nVegan men shoes: {len(vegan)} -> {out}')
