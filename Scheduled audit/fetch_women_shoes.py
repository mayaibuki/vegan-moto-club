"""
fetch_women_shoes.py
Fetch REV'IT! women's shoes, dedup by base name, then visit each product page
...
r = requests.get(
    'https://revitsport.com/en-us/collections/women-motorcycle-shoes/products.json',
...
print(f'Unique shoes: {len(unique)}')
...
out = os.path.join(os.path.dirname(__file__), 'women_shoes.json')
with open(out, 'w') as f:
    json.dump(vegan, f, indent=2, ensure_ascii=False)
print(f'\nVegan women shoes: {len(vegan)} -> {out}')
