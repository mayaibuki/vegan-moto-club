"""
fetch_men_pants.py
Fetch REV'IT! men's pants, dedup by base name, then visit each product page
...
r = requests.get(
    'https://revitsport.com/en-us/collections/men-motorcycle-pants/products.json',
...
out = os.path.join(os.path.dirname(__file__), 'men_pants.json')
with open(out, 'w') as f:
    json.dump(vegan, f, indent=2, ensure_ascii=False)
print(f'\nVegan men pants: {len(vegan)} -> {out}')
